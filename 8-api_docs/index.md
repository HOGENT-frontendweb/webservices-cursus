...
2. We maken een nieuwe Swagger configuratie aan met `DocumentBuilder`. De `DocumentBuilder` biedt een API in kettingvorm om de OpenAPI specificatie te configureren.
...
6. We voegen ondersteuning toe voor Bearer authentication (JWT). Dit voegt een Authorize knop toe in de Swagger UI.
   - Zo kan je snel een soort van "aanmelden" zonder front-end.
7. Maakt het configuratie-object dat wordt gebruikt om de volledige OpenAPI specificatie te genereren
...
Start je applicatie en navigeer naar <http://localhost:3000/docs>. Je zou de Swagger UI moeten zien met de basis configuratie. Je ziet dat er reeds heel wat endpoints zijn toegevoegd, maar zonder documentatie. En dat zonder enkele extra code te schrijven, leuk toch?

De JSON specificatie is beschikbaar op <http://localhost:3000/docs-json>.

?> **Tip:** de JSON kan je kopiëren en plakken in de [Swagger Editor](https://editor.swagger.io/) om de gegenereerde documentatie te checken op onjuistheden.
...
We gebruiken `type: () => [PlaceResponseDto]` om aan te geven dat `items` een array is van `PlaceResponseDto` objecten. Dit is nodig om een correcte OpenAPI specificatie te genereren voor arrays van objecten.

### Request DTO's met nestjs-swagger-dto

Voor request DTO's gebruiken we `nestjs-swagger-dto`. Deze package combineert `class-validator` decorators met Swagger documentatie. Op die manier kunnen we validatie en documentatie (wat vaak hetzelfde zegt) in één stap doen:
...
2. `@IsString` valideert dat de waarde een string is en documenteert dit automatisch in Swagger. We geven ook de naam en maximale lengte mee. Standaard moet een string ingevuld zijn en kan die niet leeg zijn.
...
Voor update DTO's kunnen we vaak de create DTO hergebruiken:
...
8. Plak het token in het veld en klik op **Authorize** en vervolgens op **Close**.
...
## Oefening 1 - Transactions en users documenteren

1. Documenteer de volledige `TransactionModule`:
...
- [ ] Alle request DTO's gebruiken waar mogelijk `nestjs-swagger-dto` voor validatie + documentatie
...
- Voeg pagination toe aan je lijst endpoints en documenteer deze correct met query parameters
  - Dit wordt niet als extra aangerekend, maar is wel een goede oefening
